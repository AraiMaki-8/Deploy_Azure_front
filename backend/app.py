...
#     date = data.get('date')
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "あなたは歴史の専門家であり、小学校の先生です。"},
                {"role": "user", "content": f"{date}は何の日かを200文字以内にまとめてやさしい言葉で教えてください。"}
            ],
        ...
        # 応答をもとに画像生成プロンプトを作成
        image_prompt = f"{chatgpt_reply}に関連する画像をかわいらしく生成してください。"

        # DALL·E APIを使って画像を生成
        response_image = client.images.generate(
            model="dall-e-3",
            prompt=image_prompt,  # ChatGPTの応答に基づくプロンプト←これがないと表示されない
            n=1, # 生成する画像の数
            size="1024x1024"  # 画像のサイズ←実際の大きさはフロントエンドで指定
        )
